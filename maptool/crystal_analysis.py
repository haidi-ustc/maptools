...
from   maptool.maptool_configuration import MStructure
...
class CrystalAnalysis:
    ...
    def __init__(self, structure, supercell=(1, 1, 1), radius=50):
        ...
        assert (isinstance(structure, MStructure))
        if supercell != (1, 1, 1):
        ...
        self._radius = radius
        mpt_log.debug('Supercell : ' + str(self._supercell))
        mpt_log.debug('Radius    : %7.2f' % self._radius)
    ...
    def close_distances(self):
        ...
        if self._pairs is None or self._distances is None:
            ...
            mpt_log.debug('Computing distances from scratch...')
            pairs_dict = {}
            distances_list = []
            index = 0
            for i, j in itertools.combinations(range(self.structure.natom), 2):
                if index % 100 == 0:
                    mpt_log.debug('Computing distance between atoms %d and %d' % (i, j))
                ret = self.structure.lattice.distance2(self.structure.frac_coords[i], self.structure.frac_coords[j],radius=self.radius)
                for k in ret:
            ...
            for i in range(self.structure.natom):
                ret = self.structure.lattice.distance2(self.structure.frac_coords[i], self.structure.frac_coords[i])
                for k in ret:
                    if str(i) not in pairs_dict:
                        pairs_dict[str(i)] = [index]
                    else:
    ...
    def all_distances(self):
        ...
        if self._all_distances is None:
            ...
            for i, j in itertools.combinations_with_replacement(range(self.structure.natom), 2):
                pair = (i, j)
                ret[pair] = self.structure.lattice.distances_in_sphere(self.structure.frac_coords[i],
                                                                           self.structure.frac_coords[j],
                                                                           radius=self.radius)
    ...
    def all_distances_by_species(self):
        ...
        ret = OrderedDict()
        atom_numbers = [x.number for x in self.structure.types_of_specie]
        ...
        for ipair in all_distances:
            elem_pair=[self.structure.species[ipair[0]], self.structure.species[ipair[1]]]
            key = tuple(sorted([x.number for x in elem_pair]))
    ...
    def structure_distances(self, delta=0.01, sigma=0.01, integrated=True):
        ...
        for spec_pair in dist_spec:
            ...
            mpt_log.debug('Pair %s' % str(spec_pair))
    ...
    def bonds_coordination(self, initial_cutoff_radius=0.8, use_laplacian=True, jump=0.01, tol=1E-15):
        ...
        while True:
            ...
            for pair in ad:
                atom1 = self.structure.species[pair[0]]
                atom2 = self.structure.species[pair[1]]
            ...
            for i in range(self.structure.natom):
                ...
                while True:
                    ...
                    for j in bonds_dict[str(i)]:
                        atom1 = self.structure.species[distances_list[j]['pair'][0]]
                        atom2 = self.structure.species[distances_list[j]['pair'][1]]
                        sum_covalent_radius = sum(covalent_radius([atom1, atom2]))
    ...
    def hardness_XX(self, initial_cutoff_radius=0.8, use_laplacian=True):
        ...
        mpt_log.debug('Atomic numbers in the structure : %s' % str(atomicnumbers))
        ...
        if f_d == 0:
            mpt_log.debug('Returning zero as hardness. f_d= %10.3f' % f_d)
            return 0.0
        ...
        for pair in diff_bonds:
            ...
            mpt_log.debug('Number of bonds for pair %s = %d' % (str(pair), num_i_j_bonds))
            tot += num_i_j_bonds

        vol = self.structure.volume

        mpt_log.debug("Structure volume: %7.3f" % vol)
        mpt_log.debug("Total number of bonds: %d" % tot)
        mpt_log.debug("Bonds: %s" % str(bonds))
